...
- Keyboard shortcuts: 
  ...
  * '1-5' to set specific grid size (2x2, 3x3, 5x1, 5x2)
...
class GPUTextureCache:
    ...
    def mark_grid_textures_as_priority(self, page_nums):
        """Mark specific page textures as priority to prevent eviction during grid view"""
    ...
    def add_texture(self, page_num, image, page_width, page_height, quality=1.0):
        ...
        # Configure texture filtering - use Linear for smooth, sharp text
        if quality >= 3.0:
            # High quality filtering with Linear for both directions for best text quality
            texture.setMinificationFilter(QOpenGLTexture.Filter.Linear)
            texture.setMagnificationFilter(QOpenGLTexture.Filter.Linear)
        else:
            # For lower quality textures, use standard filtering
            texture.setMinificationFilter(QOpenGLTexture.Filter.Linear)
            texture.setMagnificationFilter(QOpenGLTexture.Filter.Linear)
...
class ThumbnailWorker(QThread):
    """GPU-optimized asynchronous thumbnail worker that leverages existing texture cache"""
    ...
    def run(self):
        """Asynchronous GPU-optimized thumbnail generation"""
        try:
            if self.page_list:
                # Load specific pages
            ...
            # Process thumbnails in time-sliced chunks to prevent CPU blocking
            chunk_size = 3  # Process 3 thumbnails at a time
            for i in range(0, len(pages_to_process), chunk_size):
                ...
                for page_num in chunk:
                    ...
                    try:
                        # OPTIMIZATION 1: Check if GPU texture already exists
                        if (self.gpu_widget and hasattr(self.gpu_widget, 'texture_cache')):
                            texture = self.gpu_widget.texture_cache.get_texture(page_num)
                            if texture:
                                # Convert existing GPU texture to thumbnail - much faster than re-rendering
                                self._create_thumbnail_from_cache(page_num, texture)
                                continue
                        
                        # OPTIMIZATION 2: Use ultra-low quality for thumbnail generation
                        if page_num < 0 or page_num >= self.pdf_doc.page_count:
                            continue
                            
                        page = self.pdf_doc[page_num]
                        # Ultra-low quality matrix for fast rendering (0.4 instead of 0.6)
                        mat = fitz.Matrix(0.4, 0.4)  # Much lower quality for speed
                        
                        # Fast pixmap generation without alpha channel for speed
                        pix = page.get_pixmap(matrix=mat, alpha=False, annots=False)  # Skip annotations for speed
                        
                        # Use JPEG for faster processing instead of PNG
                        img_data = pix.tobytes("jpeg", jpg_quality=70)  # Lower quality JPEG for speed
                ...
                # Time slice: pause between chunks to prevent CPU blocking
                time.sleep(0.005)  # 5ms pause between chunks
    ...
    def _create_thumbnail_from_cache(self, page_num, texture):
        """Create thumbnail from existing GPU texture - ultra fast"""
        try:
            # For now, fall back to regular rendering since GPU texture extraction is complex
            # This is a placeholder for future GPU-to-thumbnail optimization
            pass
        except Exception as e:
            print(f"GPU thumbnail extraction error: {e}")
...
class GPUPDFWidget(QOpenGLWidget):
    ...
    def paintGL(self):
        try:
            ...
            # CRITICAL: During any user interaction (panning, zooming), completely pause ALL texture processing
            # Also pause if renderer is actively processing to prevent CPU blocking
            ...
            is_actively_interacting = (getattr(self, '_fast_zoom_mode', False) or 
                                     ...
                                     renderer_busy)  # Include renderer busy state
            
            if is_actively_interacting:
                # COMPLETE FREEZE: Zero texture processing during any active interaction or when renderer is busy
    ...
    def show_context_menu(self, position):
        ...
        if action == next_page_action:
            ...
            self.zoom_in(QPoint(position.x(), position.y()))
        elif action == zoom_out_action:
            self.zoom_out(QPoint(position.x(), position.y()))
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            self._active_panning = True    # Flag for active panning optimizations
    ...
    def mouseMoveEvent(self, event):
        ...
        # CRITICAL CPU OPTIMIZATION: If renderer is busy processing textures, defer the pan update
        # This prevents the main thread from waiting on texture operations during panning
        if hasattr(self, 'renderer') and self.renderer and hasattr(self.renderer, 'is_busy'):
            if self.renderer.is_busy():
                # Renderer is processing - defer this pan update to avoid CPU blocking
                if not hasattr(self, '_deferred_pan_timer'):
                    self._deferred_pan_timer = QTimer()
                    self._deferred_pan_timer.setSingleShot(True)
                    self._deferred_pan_timer.timeout.connect(lambda: self.mouseMoveEvent(event))
                
                # Try again in 5ms when renderer might be free
                if not self._deferred_pan_timer.isActive():
                    self._deferred_pan_timer.start(5)
                return
        
        if self.is_panning:
            delta = event.position() - self.last_mouse_pos
            
            # Skip very small mouse movements to reduce unnecessary updates
            ...
            if not hasattr(self, '_pan_update_timer'):
                ...
                self._pan_update_timer.timeout.connect(self.update)
            
            # Only update if timer is not already running (throttle to ~60 FPS)
            if not self._pan_update_timer.isActive():
                self._pan_update_timer.start(16)  # ~60 FPS (1000ms/60fps = 16.67ms)
    
    def mouseReleaseEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            self.is_panning = False
            self._active_panning = False   # Disable panning optimizations
            self._interaction_mode = False  # Disable interaction mode
            
            # Stop the pan update timer if it's running
            if hasattr(self, '_pan_update_timer'):
                self._pan_update_timer.stop()
            
            # Immediate update after panning ends for final position
            self.update()
...
class PDFViewer(QMainWindow):
    ...
    def create_toolbar(self):
        ...
        self.grid_size_combo.addItems(["2x2", "3x3", "5x1", "5x2"])
    ...
    def setup_shortcuts(self):
        ...
        QShortcut(QKeySequence("3"), self, lambda: self.set_grid_size("5x1"))  # Moved 5x1 to key 3
        QShortcut(QKeySequence("4"), self, lambda: self.set_grid_size("5x2"))  # Moved 5x2 to key 4
...
Press 'L' to cycle through modes."""
    ...
    def _rebuild_grid_size_combo(self, current_text=None):
        ...
        if current_text and current_text != "4x4":  # Skip 4x4 if it was selected
    ...
    def _create_placeholder_thumbnails(self, start_page, end_page):
        ...
        try:
            ...
            placeholder_img.fill(QColor(60, 60, 60))  # Dark gray placeholder
            
            # Add page number text to placeholder
            ...
            for page_num in range(start_page, end_page + 1):
                ...
                page_painter = QPainter(page_placeholder)
                page_painter.setPen(QColor(180, 180, 180))
                page_painter.setFont(QFont("Arial", 12, QFont.Weight.Bold))
                
                # Draw page number in center
                rect = page_placeholder.rect()
                page_painter.drawText(rect, Qt.AlignmentFlag.AlignCenter, str(page_num + 1))
                page_painter.end()
            ...
            painter.end()
            print(f"Created {end_page - start_page + 1} placeholder thumbnails instantly")
    ...
    def change_grid_size(self, size_text):
        ...
        # Block 4x4 grid completely
        if size_text == "4x4":
            self.status_bar.showMessage("4x4 grid disabled - switching to 3x3", 2000)
            # Switch to 3x3 instead
            size_text = "3x3"
